...
TOKEN = "8459759349:AAHZtkKUgq9qE_qcVy1_VgNIjPifJqjkTHo"
...
def schedule_job(application: Application, chat_id: int, job: dict):
    ...
    job_time = job["time"].replace(tzinfo=EGYPT_TZ)
    ...
    logging.info("Scheduled job %s for chat %s at %s on days %s (Egypt time)", job["id"], chat_id, job["time"], days_tuple)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("hour_"):
        ...
        hour_12 = int(parts[1])
        chat_id = int(parts[2])
        USER_STATE[user_id].update({"hour_12": hour_12, "step": "wait_minute_group"})
        
        keyboard = [
            [
                InlineKeyboardButton("00-09", callback_data=f"mingroup_0_{chat_id}"),
                InlineKeyboardButton("10-19", callback_data=f"mingroup_10_{chat_id}"),
                InlineKeyboardButton("20-29", callback_data=f"mingroup_20_{chat_id}")
            ],
            [
                InlineKeyboardButton("30-39", callback_data=f"mingroup_30_{chat_id}"),
                InlineKeyboardButton("40-49", callback_data=f"mingroup_40_{chat_id}"),
                InlineKeyboardButton("50-59", callback_data=f"mingroup_50_{chat_id}")
            ],
    ...
    if data.startswith("mingroup_"):
        parts = data.split("_")
        start_minute = int(parts[1])
        chat_id = int(parts[2])
        
        minutes_kb = []
        for i in range(start_minute, start_minute + 10):
            minutes_kb.append(InlineKeyboardButton(f"{i:02d}", callback_data=f"minute_{i}_{chat_id}"))
        
        keyboard = [minutes_kb[i:i+5] for i in range(0, len(minutes_kb), 5)]
        keyboard.append([InlineKeyboardButton("🔙 رجوع", callback_data=f"hour_{USER_STATE[user_id]['hour_12']}_{chat_id}")])
        await query.edit_message_text(f"اختر الدقيقة ({start_minute:02d}-{start_minute+9:02d}):", reply_markup=InlineKeyboardMarkup(keyboard))
        return
